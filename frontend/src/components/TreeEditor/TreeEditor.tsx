...
const TreeEditor = ({
  ...
  let lastMovedNodePosition = { x: 100, y: 100 };
  ...
  const addDefaultPorts = (node: BasicNodeModel | TagNodeModel) => {
    ...
    const actionFrame = getActionFrame(nodeName);
  ...
  const deleteLastClickedNode = () => {
    if (model.current && lastClickedNodeId.current) {
      const node: NodeModel = model.current.getNode(lastClickedNodeId.current);
      if (node) {
        node.remove();
